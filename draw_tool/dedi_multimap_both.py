...
for i in range(len(sys.argv)-1):
    # print(len(xArr[i])/15)
    print(yArr[i][-1])
    plt.plot(xArr[i][:mainLen],yArr[i][:mainLen], label=typeArr[i], linewidth=4, marker=markerTable[typeArr[i]], color=colorTable[typeArr[i]], markevery=int(len(xArr[i])/15), markersize=12, linestyle=lineTable[typeArr[i]])
...
left,bottom,width,height = 0.73,0.28,0.25,0.25
