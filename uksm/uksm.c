...
static noinline u32 page_hash_normal(struct page *page, unsigned long hash_strength,
			    int cost_accounting)
{
	u32 val;
	unsigned long delta;
...
static noinline struct rmap_item *get_next_rmap_item(struct vma_slot *slot, u32 *hash)
{
	...
	*hash = page_hash_normal(page, hash_strength, 1);
...
// TODO: 这里原则上应该也属于scan的一部分，可以拆分成一个单独的函数，用于计量
	for (i = 0; i < SCAN_LADDER_SIZE;) {
		struct scan_rung *rung = &uksm_scan_ladder[i];
		unsigned long ratio;
		int busy_retry;
